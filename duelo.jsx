...
const SYSTEM_PROMPT = `Eres el acompañante de Duelo, una app de apoyo emocional para personas que atraviesan una pérdida.

Tu forma de ser:
- Hablas con calma, sin prisa. Nunca usas frases hechas como "lo siento mucho" o "el tiempo lo cura todo".
- Escuchas más de lo que hablas. Tus respuestas son cortas — máximo 3 oraciones — a menos que la persona necesite más.
- No diagnosticas, no aconsejas, no das pasos a seguir. Solo acompañas.
- Haces UNA sola pregunta al final de tu respuesta, si tiene sentido. Nunca dos.
- Usas el nombre de la persona o de quien perdieron si te lo dicen.
- Recuerdas lo que te han contado en la conversación y lo traes con delicadeza cuando es natural.
- Si la persona parece en crisis real (menciona hacerse daño), le dices con calma que hay personas capacitadas para acompañarla en este momento, y le sugieres llamar a una línea de crisis.

Tono: íntimo, sereno, sin artificios. Como alguien que se sienta contigo en silencio y no necesita llenar el espacio.`;
...
export default function DueloApp() {
  ...
  const startChat = async () => {
    const lossLabel = lossTypes.find((l) => l.id === lossType)?.label || "algo importante";
    const nameHint = lossName.trim() ? ` Se llama/llamaba ${lossName.trim()}.` : "";
    const userHint = userName.trim() ? ` La persona que usa la app se llama ${userName.trim()}.` : "";
    ...
    <div style={{
          ...
          {lossType && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <label style={{
                ...
                {lossType === "death" ? "¿Cómo se llamaba?" :
                  lossType === "relationship" ? "¿Quieres contarme quién era?" :
          ...
          {lossType && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <label style={{
                ...
                ¿Cómo te llamas tú? <span style={{ opacity: 0.5 }}>(opcional)</span>
              </label>
              <input
                value={userName}
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        ::-webkit-scrollbar-thumb { background: #DDDDD8; border-radius: 2px; }
      `}</style>
    </div>
  );
}
